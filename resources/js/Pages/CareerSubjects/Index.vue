...
const deleteCareerSubject = (career_subject_id) => {
    if (confirm("¿Estás seguro?")) {
        Inertia.delete(route("careersubjects.destroy", career_subject_id));
    }
};

const changePage = (page) => {
    Inertia.get(route("careersubjects.index", { page }));
};
...
const sortedCareerSubjects = computed(() => {
    let filtered = [...props.career_subjects];
...
<template>
    <AppLayout title="Asignaciones de Asignaturas a Carreras">
        <template #header>
            <h1 class="font-semibold text-xl text-gray-800 leading-tight">
                Asignaciones de Asignaturas a Carreras
            </h1>
        </template>
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    >
                        <Link
                            :href="route('careersubjects.create')"
                        ...
                        <table class="min-w-full table-auto border-collapse">
                            <thead>
                                <tr class="bg-gray-100">
                                    ...
                                    >
                                        <button @click="setSort('career.name')">
                                            Carrera
                                            {{
                                                sort.column === "career.name"
                                                    ? sort.order === "asc"
                                                        ? "▲"
                                                        : "▼"
                                                    : ""
                            ...
                            <tbody>
                                <tr
                                    v-for="career_subject in sortedCareerSubjects"
                                    :key="career_subject.id"
                                    class="border-b border-gray-200"
                                >
                                    <td class="py-3 px-4 text-sm text-gray-800">
                                        {{
                                            career_subject.career?.name ||
                                            "Sin datos"
                                        }}
                                    </td>
                                    <td class="py-3 px-4 text-sm text-gray-800">
                                        {{
                                            career_subject.subject_id
                                                ? career_subject.subject
                                                      .subject_name
                                                : "Sin asignatura"
                                        }}
                                    </td>
                                    <td class="py-3 px-4 text-sm text-gray-800">
                                        {{
                                            career_subject.level || "Sin datos"
                                        }}
                                    </td>
