...
const loadall = (ids) => {
    axios
        ...
        .then((response) => {
            ...
            // Sincronizar datos con form.details
            form.details.career = careerdata.value.name || "";
            form.details.logo_path = careerdata.value.logo_path || "";
            form.details.subject = subjectdata.value.subject_name || "";
            form.details.subject_code = subjectdata.value.subject_code || "";
            form.details.level = careersubjectdata.value.level || "";
            form.details.curricular_org_unit =
                subjectdata.value.curricular_org_unit || "";
            form.details.academic_period = academicperioddata.value.name || "";
            form.details.modality = careerdata.value.modality || "";
            form.details.teacher = teacherdata.value.name || "";
            form.details.email = teacherdata.value.email || "";
            form.details.credits = subjectdata.value.credits || "";
            form.details.totalHours = totalHours.value || "";
            form.details.contact_teacher =
                subjectdata.value.contact_teacher || "";
            form.details.practical_teaching =
                subjectdata.value.practical_teaching || "";
            form.details.self_employment =
                subjectdata.value.self_employment || "";
            form.details.practical_self_employment =
                subjectdata.value.practical_self_employment || "";
            form.details.generic_competences =
                genericCompetencesString.value || "";
            form.details.research_lines_career =
                careerdata.value.research_lines || "";
            form.details.description_integrative_project =
                subjectdata.value.description_integrative_project || "";

            form.details.description_subject =
                subjectdata.value.description || "";
            form.details.objectives_subject =
                subjectdata.value.objectives || "";
            form.details.gral_learning_outcome =
                subjectdata.value.gral_learning_outcome || "";
            form.details.teachinglearningstrategies =
                teachinglearningstrategiesString.value || "";
            ...
            form.details.totalHours2 = totalHours2.value || "";
            form.details.totalPartial1 = totalPartial1.value || "";
            form.details.totalPartial2 = totalPartial2.value || "";

            // Sincronización de arrays
            form.details.prerequisites =
                subjectrequisitesdata.value.map((req) =>
                    getSubjectName(req.prerequisites)
                ) || [];
            form.details.prerequisites_code =
                subjectrequisitesdata.value.map((req) =>
                    getSubjectCode(req.prerequisites)
                ) || [];
            form.details.corequisites =
                subjectrequisitesdata.value.map((req) =>
                    getSubjectName(req.corequisites)
                ) || [];
            form.details.corequisites_code =
                subjectrequisitesdata.value.map((req) =>
                    getSubjectCode(req.corequisites)
                ) || [];

            form.details.competence_specif_learningoutcomes =
                specificcompetencesdata.value.map(
                    (comp) => comp.Learning_outcomes
                ) || [];
            form.details.competence_specif_contribution_level =
                specificcompetencesdata.value.map(
                    (comp) => comp.contribution_level
                ) || [];

            form.details.curricularunit_unit =
                curricularunitsdata.value.map((unit) => unit.unit) || [];
            form.details.curricularunit_name =
                curricularunitsdata.value.map((unit) => unit.name) || [];
            form.details.curricularunit_learning_outcomes =
                curricularunitsdata.value.map(
                    (unit) => unit.learning_outcomes
                ) || [];

            form.details.teachingcontent_schedule =
                teachingcontentsdata.value.map((content) => content.schedule) ||
                [];
            form.details.teachingcontent_content =
                teachingcontentsdata.value.map((content) => content.content) ||
                [];
            form.details.teachingcontent_contact_teacher =
                teachingcontentsdata.value.map(
                    (content) => content.contact_teacher
                ) || [];
            form.details.teachingcontent_contact_teacher_duration =
                teachingcontentsdata.value.map(
                    (content) => content.contact_teacher_duration
                ) || [];
            form.details.teachingcontent_practical_teaching =
                teachingcontentsdata.value.map(
                    (content) => content.practical_teaching
                ) || [];
            form.details.teachingcontent_practical_teaching_duration =
                teachingcontentsdata.value.map(
                    (content) => content.practical_teaching_duration
                ) || [];
            form.details.teachingcontent_self_employment =
                teachingcontentsdata.value.map(
                    (content) => content.self_employment
                ) || [];
            form.details.teachingcontent_self_employment_duration =
                teachingcontentsdata.value.map(
                    (content) => content.self_employment_duration
                ) || [];
            form.details.teachingcontent_practical_self_employment =
                teachingcontentsdata.value.map(
                    (content) => content.practical_self_employment
                ) || [];
            form.details.teachingcontent_practical_self_employment_duration =
                teachingcontentsdata.value.map(
                    (content) => content.practical_self_employment_duration
                ) || [];

            /*form.details.evaluation_unit =
                evaluationsdata.value.map((evalu) => evalu.unit) || [];*/

            form.details.evaluation_unit = evaluationsdata.value.map(
                (evalu) => {
                    // Busca la unidad curricular correspondiente
                    const matchingUnit = curricularunitsdata.value.find(
                        (unit) => unit.id === evalu.unit
                    );

                    // Si se encuentra una unidad curricular coincidente, devuelve el valor de unit.unit
                    return matchingUnit ? matchingUnit.unit : null; // o algún otro valor por defecto si no hay coincidencia
                }
            );

            form.details.evaluation_Learning_outcomes =
                evaluationsdata.value.map((evalu) => evalu.Learning_outcomes) ||
                [];
            form.details.evaluation_type =
                evaluationsdata.value.map((evalu) => evalu.type) || [];
            form.details.evaluation_criteria =
                evaluationsdata.value.map((evalu) => evalu.criteria) || [];
            form.details.evaluation_activity =
                evaluationsdata.value.map((evalu) => evalu.activity) || [];
            form.details.evaluation_first_partial =
                evaluationsdata.value.map((evalu) => evalu.first_partial) || [];
            form.details.evaluation_second_partial =
                evaluationsdata.value.map((evalu) => evalu.second_partial) ||
                [];

            form.details.bibliography_type =
                bibliographiesdata.value.map((biblio) => biblio.type) || [];
            form.details.bibliography_author =
                bibliographiesdata.value.map((biblio) => biblio.author) || [];
            form.details.bibliography_year =
                bibliographiesdata.value.map((biblio) => biblio.year) || [];
            form.details.bibliography_title =
                bibliographiesdata.value.map((biblio) => biblio.title) || [];
            form.details.bibliography_editorial =
                bibliographiesdata.value.map((biblio) => biblio.editorial) ||
                [];
            form.details.bibliography_format =
                bibliographiesdata.value.map((biblio) => biblio.format) || [];
            form.details.bibliography_code =
                bibliographiesdata.value.map((biblio) => biblio.code) || [];
            form.details.bibliography_link =
                bibliographiesdata.value.map((biblio) => biblio.link) || [];
            form.details.bibliography_uplift =
                bibliographiesdata.value.map((biblio) => biblio.uplift) || [];

            form.details.teacher_approval = teacherdata.value.name || "";
            form.details.coordinator_approval =
                coordinatordata.value.name || "";
            form.details.vice_rector_approval =
                vicerectoratedata.value.name || "";
        })
        .catch((error) => {
            console.error("Error loading subjects:", error);
...
<template>
    <!--<pre>{{ JSON.stringify(curricularunitsdata, null, 2) }}</pre>-->
                    ...
                    <div>
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="mt-4">
                                    ...
                                    <input
                                        id="subject_code"
                                        name="subject_code"
                                        v-model="form.details.subject_code"
                                        required
                                ...
                                <div>
                                    ...
                                    <input
                                        v-model="form.details.academic_period"
                                        required
                                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm"
                                    />
                                </div>
                            ...
                            <div>
                                ...
                                <textarea
                                    v-model="form.details.description_subject"
                                    rows="6"
                                    required
                            ...
                            <div class="mt-2">
                                ...
                                <textarea
                                    v-model="form.details.objectives_subject"
                                    rows="4"
                                    required
                                    class="mt-2 block w-full border border-gray-300 rounded-md shadow-sm"
                                />
                            ...
                            <div class="mt-2">
                                ...
                                <textarea
                                    v-model="form.details.gral_learning_outcome"
                                    rows="5"
                                    required
                                    class="mt-2 block w-full border border-gray-300 rounded-md shadow-sm"
                                />
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                <div class="row-span-2 col-span-1 w-full mt-10">
                                    <div>
                                        ...
                                        <input
                                            v-for="(
                                                curricularunit, index
                                            ) in form.details
                                                .curricularunit_unit"
                                            :key="index"
                                            v-model="
                                                form.details
                                                    .curricularunit_unit[index]
                                            "
                                            required
                                            class="mt-2 block w-full border border-gray-300 rounded-md shadow-sm"
                                        />
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-10 gap-0">
                                <div class="mt-7">
                                    ...
                                    <textarea
                                        v-for="(teachingcontent, index) in form
                                            .details.teachingcontent_schedule"
                                        :key="index"
                                        v-model="
                                            form.details
                                                .teachingcontent_schedule[index]
                                        "
                                        rows="6"
                                        required
                                ...
                                <div class="mt-11">
                                    ...
                                    <textarea
                                        v-for="(teachingcontent, index) in form
                                            .details.teachingcontent_content"
                                        :key="index"
                                        v-model="
                                            form.details
                                                .teachingcontent_content[index]
                                        "
                                        rows="6"
                                        required
                                        class="mt-0 block w-full border border-gray-300 rounded-md shadow-sm text-xs p-0 m-0 leading-tight"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-10 gap-0">
                                <div class="mt-8">
                                    ...
                                    <textarea
                                        v-for="(evaluation, index) in form
                                            .details.evaluation_unit"
                                        :key="index"
                                        v-model="
                                            form.details.evaluation_unit[index]
                                        "
                                        rows="6"
                                        required
                                        class="mt-0 block w-full border border-gray-300 rounded-md shadow-sm text-xs p-0 m-0 leading-tight"
                                ...
                                <div class="mt-8">
                                    ...
                                    <textarea
                                        ...
                                        :key="index"
                                        v-model="
                                            form.details.evaluation_type[index]
                                        "
                                        rows="6"
                                        required
                                        class="mt-0 block w-full border border-gray-300 rounded-md shadow-sm text-xs p-0 m-0 leading-tight"
                                ...
                                <div class="mt-4">
                                    ...
                                    <textarea
                                        v-for="(evaluation, index) in form
                                            .details.evaluation_first_partial"
                                        :key="index"
                                        v-model="
                                            form.details
                                                .evaluation_first_partial[index]
                                        "
                                        rows="6"
                                        class="mt-0 block w-full border border-gray-300 rounded-md shadow-sm text-xs p-0 m-0 leading-tight"
                                    />
                                ...
                                <div class="mt-4">
                                    ...
                                    <textarea
                                        v-for="(evaluation, index) in form
                                            .details.evaluation_second_partial"
                                        :key="index"
                                        v-model="
                                            form.details
                                                .evaluation_second_partial[
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-10 gap-0">
                                <div class="mt-10">
                                    ...
                                    <textarea
                                        v-for="(bibliography, index) in form
                                            .details.bibliography_type"
                                        :key="index"
                                        v-model="
                                            form.details.bibliography_type[
                                                index
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <input
                                        v-model="form.details.teacher_approval"
                                        required
                                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm"
                                    />
                                </div>
